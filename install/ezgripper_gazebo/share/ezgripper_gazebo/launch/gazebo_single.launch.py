...
def generate_launch_description():
    ...
    # 2. Load URDF (Single Mount Standalone)
    xacro_file = os.path.join(pkg_ezgripper_description, 'urdf', 'ezgripper_single_mount_standalone.urdf.xacro')
    
    # WRAPPED IN ParameterValue FOR JAZZY COMPLIANCE
    ...
    # NOTE: Single gripper uses 'ezgripper_controller' (singular) 
    # Check your controllers.yaml to confirm the name if this fails!
    ...
    return LaunchDescription([
        set_gz_resource_path, # <--- Applies the mesh fix
        gz_sim,
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[load_jsb, load_gripper_ctrl],
            )
        ),
    ])
